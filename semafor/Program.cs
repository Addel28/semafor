...
class Program
{
    ...
    static async Task Main(string[] args)
    {
        int resourceCount = 3;
        semaphore = new SemaphoreSlim(resourceCount);
        //int MaxTraidCount = Environment.ProcessorCount;
    ...
    static async Task AccessResourceAsync()
    {
        ...
        Console.WriteLine($"Поток {i} ожидает доступа к ресурсу.");
        await semaphore.WaitAsync();
        await Task.Delay(500);
        Console.WriteLine($"Поток {i} получил доступ к ресурсу.");
        await Task.Delay(200);

        Console.WriteLine($"Поток {i} освободил ресурс.");
        semaphore.Release();
    }
}
